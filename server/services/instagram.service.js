...
class InstagramService {
	...
	async getTag(name) {
		const url = `${
			this.INSTAGRAM_API_TAGS
		}/${name}/media/recent?access_token=${this.INSTAGRAM_ACCESS_TOKEN}`;
		return await axios
			.get(url)
            .then(response => response.data)
            .catch(err => {
				throw new errors.NotFoundError(
					`Can't find tag '${name}', Err:: ${err}`
				);
			});
	}
	...
	async oauth() {
		const url = `${this.INSTAGRAM_API_OAUTH}?client_id=${
